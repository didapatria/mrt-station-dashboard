...
import { User, Mail, Shield, Calendar, Lock, Eye, EyeOff, Globe, Palette, Bell, Monitor } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const profileItems = [
    ...
    { icon: Calendar, label: t("profile.memberSince"), value: new Date(user.createdAt).toLocaleDateString(i18n.language === "id" ? "id-ID" : "en-US", { year: "numeric", month: "long", day: "numeric" }) },
  ];

  const preferencesItems = [
    { icon: Globe, label: t("nav.menu") + " Language", value: i18n.language === "id" ? "Indonesia" : "English" },
    { icon: Palette, label: "Theme", value: theme === "dark" ? "Dark" : "Light" },
    { icon: Monitor, label: "Platform", value: navigator.userAgent.includes("Mac") ? "macOS" : "Desktop" },
    { icon: Bell, label: "Notifications", value: "Real-time (SSE)" },
  ];

  const frontendStack = [
    "React 19", "TypeScript", "Vite 8", "TanStack Query", "Zustand",
  ...
  const backendStack = [
    ...
    "PostgreSQL", "JWT", "bcrypt", "Zod", "Swagger/OpenAPI",
    "SSE", "Rate Limiting",
  ];
  const infraStack = ["Docker", "docker-compose", "Nginx", "GitHub Actions", "PWA", "Playwright"];
  const testingStack = ["Vitest", "React Testing Library", "Supertest", "Playwright"];
              ...
              <CardContent className="pt-6">
                <div className="space-y-3">
                  {profileItems.map((item) => {
                    ...
                    return (
                      <div key={item.label} style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: "0.75rem", padding: "0.5rem 0" }}>
              ...
              <CardContent className="pt-6">
                <div className="space-y-3">
                  {preferencesItems.map((item) => {
                    const Icon = item.icon;
                    return (
                      <div key={item.label} style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "space-between", padding: "0.5rem 0" }}>
          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>{frontendStack.length + backendStack.length + infraStack.length + testingStack.length} technologies</CardDescription>
              </CardHeader>
              <CardContent>
                <Tabs defaultValue="frontend">
                  <TabsList className="mb-4 w-full">
                    ...
                    <TabsTrigger value="infra" className="flex-1">Infra</TabsTrigger>
                    <TabsTrigger value="testing" className="flex-1">Testing</TabsTrigger>
                  </TabsList>
                  <TabsContent value="frontend">
                    <div className="flex flex-wrap gap-2">{frontendStack.map((tech) => <Badge key={tech} variant="outline">{tech}</Badge>)}</div>
                  </TabsContent>
                  <TabsContent value="backend">
                    <div className="flex flex-wrap gap-2">{backendStack.map((tech) => <Badge key={tech} variant="outline">{tech}</Badge>)}</div>
                  </TabsContent>
                  <TabsContent value="infra">
                    <div className="flex flex-wrap gap-2">{infraStack.map((tech) => <Badge key={tech} variant="outline">{tech}</Badge>)}</div>
                  </TabsContent>
                  <TabsContent value="testing">
                    <div className="flex flex-wrap gap-2">{testingStack.map((tech) => <Badge key={tech} variant="outline">{tech}</Badge>)}</div>
                  </TabsContent>
                </Tabs>
              </CardContent>
            </Card>
