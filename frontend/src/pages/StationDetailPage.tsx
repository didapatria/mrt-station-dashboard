...
export default function StationDetailPage() {
  ...
  if (!station) return <p className="text-muted-foreground">Station not found.</p>;

  return (
    <div>
      <Button variant="ghost" size="sm" onClick={() => navigate("/stations")} className="mb-4">
        <ArrowLeft className="h-4 w-4 mr-2" />{t("stations.title")}
      </Button>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="grid gap-6 lg:grid-cols-[1fr_350px]">
          ...
          <div className="space-y-6">
                ...
                <div style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: "1rem" }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: "0.5rem", marginTop: "0.25rem" }}>
                      <Badge variant={station.status === "ACTIVE" ? "success" : station.status === "MAINTENANCE" ? "warning" : "destructive"}>{station.status}</Badge>
                      <span className="text-sm text-muted-foreground">{t("stations.order")}: {station.order}</span>
                    </div>
                  </div>
                </div>
              </CardHeader>
            ...
            <Card>
              <CardHeader><CardTitle className="text-lg" style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: "0.5rem" }}><Clock className="h-4 w-4" />{t("schedules.title")} ({schedules.length})</CardTitle></CardHeader>
              <CardContent className="p-0">
                <Table>
                  ...
                  <TableBody>
                    {schedules.length === 0 ? (
                      <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground py-8">No schedules</TableCell></TableRow>
                    ) : schedules.map((s) => (
                      <TableRow key={s.id}>
                        <TableCell className="font-mono font-medium">{s.trainNumber}</TableCell>
                        <TableCell>{s.departureStation?.code} → {s.arrivalStation?.code}</TableCell>
                        <TableCell className="font-mono text-sm">{s.departureTime} — {s.arrivalTime}</TableCell>
