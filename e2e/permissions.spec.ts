...
test.describe("Permissions API", () => {
  test("GET /permissions/me returns permission list for admin", async ({ adminPage: page }) => {
  ...
  test("GET /permissions/me returns fewer permissions for operator", async ({ operatorPage: page }) => {
    const token = await page.evaluate(() => localStorage.getItem("token"));
    const res = await page.request.get(`${API_URL}/permissions/me`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  test("GET /permissions returns all permissions for admin", async ({ adminPage: page }) => {
    const token = await page.evaluate(() => localStorage.getItem("token"));
  ...
  test("GET /permissions is forbidden for operator", async ({ operatorPage: page }) => {
    const token = await page.evaluate(() => localStorage.getItem("token"));
    const res = await page.request.get(`${API_URL}/permissions`, {
      headers: { Authorization: `Bearer ${token}` },
    });
...
test.describe("Access Management Page", () => {
  test("permission matrix loads from DB for admin", async ({ adminPage: page }) => {
    await navigateTo(page, "/access");

    await expect(page.locator("text=/permission matrix/i").first()).toBeVisible({ timeout: 8000 });
    ...
    await expect(page.locator("text=/Stations|Dashboard/i").first()).toBeVisible({ timeout: 10000 });
  });

  test("operator is redirected away from /access", async ({ operatorPage: page }) => {
    await page.goto("/access");
    await page.waitForTimeout(2000);
    // Should redirect to dashboard or show no content
    const isAccessPage = await page.locator("text=/permission matrix/i").isVisible().catch(() => false);
