...
test.describe("Dashboard", () => {
  test("should display dashboard with stats and charts", async ({
    adminPage: page,
  }) => {
    ...
    // Charts
    await expect(
      page.locator("text=/Stations Breakdown|Schedules Breakdown/i").first(),
    ).toBeVisible();
  ...
  test("should not show admin-only controls for operator", async ({
    operatorPage: page,
  }) => {
    await navigateTo(page, "/dashboard");
    await page.waitForTimeout(2000);
  ...
  test("should switch data tabs and use export buttons", async ({
    adminPage: page,
  }) => {
    await navigateTo(page, "/dashboard");
    ...
    const exportStationsBtn = page.getByRole("button", {
      name: /export stations/i,
    });
    await expect(exportStationsBtn).toBeVisible();

    const exportSchedulesBtn = page.getByRole("button", {
      name: /export schedules/i,
    });
